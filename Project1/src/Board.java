...
public class Board {
	...
	private int[][] boardState; // The board. 0s represent empty spaces, 1's
								// represent player 1's pieces, 2's represent
								// player 2's pieces
	private int rows; // The number of rows on the board
	private int cols; // The number of cols on the board
	private boolean player1popused; // Boolean for if player 1 has used their
									// pop
	private boolean player2popused; // Boolean for if player 2 has used their
									// pop
	private int turn; // Whos turn it is. 1 represents player 1's turn, 2
						// represents player 2's turn. It represents the player
						// to make the next move
						// In regards to turn: If player1 is going first, the
						// empty board's turn is
						// P1. It represents that P1 can make the next move

	private int n; // The number of pieces in a row to win
	private double evaluationValue; // The evaluation value of the current board
									// state
	private ArrayList<Board> nextPossibleBoards; // All the possible boards that
													// can come from this board
	...
	// Constructor for board. Initialize most variables
	// rows is the number of rows of the board
	// cols is the number of cols of the board
	...
	// Evaluate the state of a board. currentDepth is the depth of the board
	// (where the current game state is
	...
	public int evaluateToDepth(int currentDepth, int maxDepth, int alpha, int beta) {
		...
		if (currentDepth == maxDepth) {
			this.evaluate();
			// TODO This should be a double, but it would require some
			// refactoring
		...
		// Evaluate each child board. Pass currentDepth+1 because the children
		// are 1 move deeper
	...
	// Evaluation/Heuristic function. Sets the evaluationValue to the numeric
	...
	public void evaluate() {
		...
		int arraySize = n + 2;
		// TODO There very well may be an issue with chains not being put in the
		// right spot in the array, spot + 1
		...
		if ((numberOfHConnectionsP2[n] > 0) || (numberOfVConnectionsP2[n] > 0) || (numberOfDLConnectionsP2[n] > 0)
				|| (numberOfDRConnectionsP2[n] > 0)) {
			functionResult += (Integer.MIN_VALUE / 4);
			evaluationValue = functionResult;
		}

		if ((numberOfHConnectionsP1[n] > 0) || (numberOfVConnectionsP1[n] > 0) || (numberOfDLConnectionsP1[n] > 0)
				|| (numberOfDRConnectionsP1[n] > 0)) {
	...
	// Function to check whether or not we have hit a terminal state, and for
	// which player
	...
	// Generate all possible boards that can come from this board and assign
	// their moves, turns, and board states, and if a pop was used
	public void generatePossibleBoards() {
		...
		// If it's player1's turn and they have already used their pop, don't
		// generate pop moves
		if (this.player1popused && this.turn == P1) {
			return;
		}

		// If it's player2's turn and they have already used their pop, don't
		// generate pop moves
	...
	// Return the index of the first open row in the passed column (closet to
	...
	// Update the board state of this board to reflect the execution of the
	// passed move and update the turn
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				if (boardState[i][j] == player) {
					// If there is a space to the left, then increment the
					...
					// If there is a space to the right, then increment the
					// number of open spots
					if (j < cols - 1) {
						if (boardState[i][j + 1] == 0)
							openSpots++;
					}
					// Add the number of open spots to the corresponding place
					// in currentFound
					// Using the number of openSpots to represent the value of a
					// connected row
		...
		for (int currCol = 0; currCol < cols; currCol++) {
			for (int currRow = 0; currRow < rows; currRow++) {
				if (boardState[currRow][currCol] == player) {
					...
					// System.err.println("Adding to openspots.
	...
	public int[] findDiagonalConnections(int[][] boardState, int[] oldNumConnections, int type, int player) {
		int[][] recordedChart = new int[rows][cols]; // Chart to see which
														// places have been
														// visited already. 0
			...
			for (int currRow = rows - 1; currRow >= 0; currRow--) {
				for (int currCol = 0; currCol < cols; currCol++) {
					...
					if ((boardState[currRow][currCol] == player) && (recordedChart[currRow][currCol] == UNCHECKED)) {
						...
						// See if we have room such that we even need to check
						// for an opening to the bot right
						...
						if ((currRow > 0) && (currCol > 0))// Check that
															// iterating won't
															// create an out of
															// bounds error
						{
							// Use iterators because we cannot skip parts of the
							// loop
							rowIterator = currRow;
							colIterator = currCol;
							// This checks up one and to the left
							while ((boardState[rowIterator - 1][colIterator - 1] == player)
									&& (recordedChart[rowIterator - 1][colIterator - 1] == UNCHECKED)) {
						...
						// Check if we have room for an open spot to the upper
						// left
						if ((rowIterator > 0) && (colIterator > 0)) {
						...
						rowIterator = 0;
						colIterator = 0;
						// Add found diagonal to list and reset tracking
						// variables
						if (lengthCounter > n) { // TODO: Verify this change
													// works
							currentFound[n + 1] += openSpots;
						} else {
							currentFound[lengthCounter] += openSpots;
			...
			for (int currRow = (rows - 1); currRow >= 0; currRow--) {
				for (int currCol = 0; currCol < cols; currCol++) {
					...
					if ((boardState[currRow][currCol] == player) && (recordedChart[currRow][currCol] == UNCHECKED)) {
						...
						recordedChart[currRow][currCol] = CHECKED;
						// See if we have room such that we even need to check
						// for an opening to the bot left
						...
						lengthCounter++;
						// Iterate along diagonal
						if ((currRow > 0) && (currCol < cols - 1))// Check that
																	// iterating
																	// won't
																	// create an
																	// out of
